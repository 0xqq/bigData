...
export default class TaskConfigPM extends PmMainBase{
  ...
  constructor( name,id ) {
    ...
    this.deleteMethod = ProcessAPI.deleteTask ;
    this.updateMethod = ProcessAPI.saveConfig ;

    //数据加载
    this.pagePM = new PagePm(ProcessAPI.queryTaskList) ;
  ...
  /*查询列表*/
  queryList(){
     this.pagePM.queryList(this.data.searchForm,{type:"2"},(datas)=>{
  ...
  changeToSourceLink(item){
    NormalUtils.changeRouter(PathConst.DH_DIS_TASK_CREATE, {id:item.id,name:item.name} ) ;
  }
