...
export default class TaskMoniorPM extends FormPmBase{
  ...
  constructor( name,id ) {
    ...
    this.pagePM = new PagePm(ProcessAPI.queryQualityMonitorList,1,"list");
    // this.pagePM.sortAtt = "value" ;
    // this.pagePM.noFilter = true ;//这的格式和通用的不同

    this.detailPM = new PagePm(ProcessAPI.queryQualityMonitorListDetail,1,"list") ;
    ...
    delete send.runTime ;
     super.queryList(send,(datas)=>{
       for(let item of datas){
         ...
         if( item.endTime )
           item.endTime = NormalUtils.dateFormat(parseInt(item.endTime)) ;
         item.statusName = "";
