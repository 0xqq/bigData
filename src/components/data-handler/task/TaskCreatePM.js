...
export default class TaskManagerPM extends FormPmBase{
  ...
  executeSubmit( data ){
    ...
    let _original = JSON.parse(sessionStorage.getItem('checkedValue'));
    let _sql = JSON.parse(sessionStorage.getItem('sqlValue'));

    console.log(_original + _sql);


    let twoValue ={
      original: _original == true ? 'Y': 'N',
      sql:_sql
    };

    console.log(twoValue);

    Object.assign(data.model.transformation.step[0],twoValue);
