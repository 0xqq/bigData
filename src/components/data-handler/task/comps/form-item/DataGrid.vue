  ...
  export default {
    ...
    watch:{
      ...
      sqlValue:function (val) {
        sessionStorage.setItem('sqlValue',JSON.stringify(this.sqlValue));
      },
      checked:function (val) {
        sessionStorage.setItem('checkedValue',JSON.stringify(this.checked));
      }
    },
    ...
    <v-group-iwt class="size">
      ...
      <h-group-iwt class="tj-diy-form-item-lb2" v-if="label||hasGet" >
        ...
        <el-checkbox v-model="checked" style="margin-left: 5px"></el-checkbox>
          <!--<div v-if="hasGet" style="margin-left: 10px">
            <el-button type="primary"
                       size="mini"
                       :disabled="checked === true ? false : true"
                       @click="">SQL编辑</el-button>
          </div>-->
      </h-group-iwt>
      <div v-if="data.desc" class="tj-diy-form-item-desc" :title="data.desc" > </div>

      <!--这是SQL编辑的-->
      <div class="table-container" v-show="checked === true">
        <el-input v-model="sqlValue" placeholder="请编辑SQL"></el-input>
      </div>

      <!--这是table的-->
      <div class="table-container" v-show="checked === false">
