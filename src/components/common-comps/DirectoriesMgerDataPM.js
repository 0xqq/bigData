...
export default class DirectoriesMgerDataPM extends QueryPmBase {
  ...
  initView(data , type=-1 ,hasAdd=true) {
    ...
    if(type===0){
      ...
      this.queryFoldMethod = DataStorageAPI.queryResourceDirs;
    }
    else if(type===1){
      this.data.selectMode = true ;
  ...
  getFoldList(result,ppath ,istree=false){
    ...
    if(this.data.hasFile && !istree && result.data.file){
  ...
  changeToFold( path ) {
    ...
    //把path的值由原来的dataInstance换成foldName；
    if(this.data.rootLuTwo === ''){
      path = this.currentData.foldName;

      console.log(path);
    }else {
      path = this.currentData.foldName +'/'+this.data.rootLuTwo;

      console.log(path);
    }
    // console.log();

    // path = this.data.rootLu;
