...
export default class DirectoriesMgerDataAnalyPM extends QueryPmBase {
  ...
  initView(data , type=-1 ,hasAdd=true) {
    ...
    console.log(path);  // '/home/zqys'
    let arr = path.split("/") ;
    console.log(arr); //["", "home", "zqys"]
    this.rootPath = "" ;
    if(arr.length>0){
      this.rootName = arr[arr.length-1] ;
      console.log(this.rootName); //zqys
      arr.pop() ;
      console.log(arr); //["", "home"]
      this.rootPath = arr.join("/") ;
      console.log(this.rootPath); // '/home'
   ...
   */
  getParams( path="" ){
    let genRoot = JSON.parse(sessionStorage.getItem('analyLu'));
    return {
