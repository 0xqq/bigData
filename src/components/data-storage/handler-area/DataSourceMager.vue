    ...
    <div class="inner">
        ...
        <div class="tab-head">
          <el-tabs type="border-card" v-model="viewData.currentState"  >
            <el-tab-pane label="即席查询" name="search" v-if="auth['4331']" >
              <span slot="search"><i class="el-icon-date"></i> 即席查询</span>
            </el-tab-pane>
            <el-tab-pane label="标签云" name="label" v-if="auth['4335']" >
        ...
        <div class="tab-content" >
          <div >
            ...
            <!--即席查询-->
            <sql-form-view v-if="auth['4331']"
                           v-show="viewData.currentState==='search'">
            </sql-form-view>

            <!--标签云-->
            <marker-form-view  v-if="auth['4335']"
        ...
        <v-gap-iwt ></v-gap-iwt>
          <!--sql查询列表-->
          <sql-data-source-list  v-if="auth['4331']"
                                 v-show="viewData.currentState==='search'" >
          </sql-data-source-list>

        <!--标签查询列表-->
        <data-source-list v-if="auth['4335']"
                          v-show="viewData.currentState==='label'" >
        </data-source-list>
