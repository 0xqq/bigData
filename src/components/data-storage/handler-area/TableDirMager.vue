<script>
  ...
  export default {
    ...
    data() {
      this.subViewPath = [
        {code:"433",path:PathConst.DS_HANDLER_CONFIG_TABLE_LIST,view:"table"},
        {code:"4311",path:PathConst.DS_HANDLER_CONFIG_FOLD,view:"fold"}] ;
      return {
  ...
  <div class="container" >
    ...
    <div class="tab-head-l">
      <el-tabs v-model="currentView" @tab-click="changeView()">
        <el-tab-pane label=" 表管理" name="table" v-if="auth['423']" >
          <span slot="label"><i class="iconfont tj-biaoge iconbtn"></i> 表管理</span>
        </el-tab-pane>
        <el-tab-pane label=" 目录管理" name="fold"  v-if="auth['4211']">
